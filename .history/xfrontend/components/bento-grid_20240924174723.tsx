...
export function FeaturesSectionDemo() {
  ...
  const [tweetUrl, setTweetUrl] = useState<string | null>(null); // State to store tweet URL
  ...
  const handleSubmit = (url: string) => {
    // Update the tweet URL state
    setTweetUrl(url);
    // Set the tweet content based on the new URL (you can add logic here to fetch the tweet content if needed)
    setTweetContent("Updated tweet content based on URL: " + url);
  };
  ...
  const features = [
    {
      ...
      skeleton: (
        <TwitterSubmission 
          value={tweetUrl ?? ""}
          onChange={handleSubmit} // Call the state update directly in the parent component
        />
      ),
    ...
    <div className="relative py-10 lg:py-20 max-w-7xl mx-auto font-sans">
      ...
      {tweetUrl && (
        <div className="mt-8 p-4 bg-gray-100 rounded-lg shadow-md">
          <TweetDemo tweetUrl={tweetUrl} /> {/* Pass the tweetUrl as prop */}
        </div>
      )}
    </div>
  );
...
const FeatureCard = ({
  ...
  return (
    <div
      className={cn(
        `p-4 sm:p-8 relative overflow-hidden h-[600px] border border-gray-200 bg-white rounded-lg shadow-lg min-h-[420px]`, 
        className
      )}
    >
      {children}
...
export const TwitterSubmission = ({
  ...
  const [tweet, setTweet] = useState(value);
