    ...
    >
      ...
      {isOpen && (
        <div className="absolute top-full left-0 w-full bg-background bg-opacity-100 dark:bg-background-dark text-foreground dark:text-white flex flex-col items-center md:hidden pb-4">
          ...
          <Link href="/tweet-anay" className="nav-item py-4 w-full text-center" style={navItemStyle} onClick={toggleMenu}>Tweet Analyzer</Link>
          <Link href="/ratedTweets" className="nav-item py-4 w-full text-center" style={navItemStyle} onClick={toggleMenu}>Analyzed Tweets</Link>
        </div>
      )}
    </nav>
  );
