      ...
      fetch(`${API_URL}/chat`, {
        ...
        .then(data => {
          if (data.response) {
            ...
            setMessages(prev => [...prev, { text: "Error: Unexpected response from server.", sender: "bot" }]);
          }
        })
        .catch(error => {
          console.error('Error:', error);
          setMessages(prev => [...prev, { text: "Error: Unable to get response from server.", sender: "bot" }]);
        });
    }
  };
        ...
        {messages.map((msg, idx) => (
          <div key={idx} className={`mb-1 flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`relative inline-block p-3 rounded-lg text-sm font-medium ${msg.sender === 'user' ? 'bg-blue-500 text-white' : 'bg-gray-300 text-gray-900'}`}>
              {msg.text}
              <span className={`absolute top-1/2 transform -translate-y-1/2 ${msg.sender === 'user'
