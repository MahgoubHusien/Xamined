...
const Chatbot = () => {
  ...
  return (
    <div className="bg-gray-100 shadow-lg rounded-lg p-6 w-full max-w-md text-charcoal flex flex-col h-[300px]">
      <div className="flex-1 overflow-y-auto mb-4 pr-2">
        {messages.map((msg, idx) => (
          <div key={idx} className={`mb-2 flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`relative inline-block p-3 rounded-lg text-sm ${msg.sender === 'user' ? 'bg-blue-500 text-white' : 'bg-gray-300 text-gray-900'}`}>
              {msg.text}
