...
import { TweetDemo } from "@/components/TweetDemo"; // Import a component to display the Tweet
  ...
  const features = [
    {
      ...
      skeleton: (
        <TwitterSubmission
          onSubmit={(url) => setTweetUrl(url)} // Update the state with the submitted tweet URL
        />
      ),
...
export const TwitterSubmission = ({ onSubmit }: { onSubmit: (url: string) => void }) => {
  const [tweet, setTweet] = useState("");

  const handleSubmit = () => {
    onSubmit(tweet); // Call the onSubmit prop with the tweet URL
  };
