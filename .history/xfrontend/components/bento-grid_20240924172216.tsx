...
const FeatureCard = ({
  ...
  return (
    <div
      className={cn(
        `p-4 sm:p-8 relative overflow-hidden h-[600px] border border-gray-200 bg-white rounded-lg shadow-lg min-h-[420px] flex items-center justify-center`, // Added flex properties for centering
        className
      )}
    >
      {children}
...
const FeatureTitle = ({ children }: { children?: React.ReactNode }) => {
  return (
    <p className="flex items-center justify-center text-center tracking-tight text-neutral-900 text-2xl font-semibold">
      {children}
    </p>
  );
};
...
const FeatureDescription = ({ children }: { children?: React.ReactNode }) => {
  return (
    <p className="text-base text-center text-neutral-600 font-normal my-2">
      {children}
    </p>
  );
};
...
export const Chatbot = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full"> {/* Flex for full height */}
      <div className="flex-1 overflow-y-auto mb-4 p-4 border border-neutral-300 rounded-md bg-white w-full">
