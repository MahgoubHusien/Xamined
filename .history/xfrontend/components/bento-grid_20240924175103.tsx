...
export function FeaturesSectionDemo() {
  ...
  const [tweetUrl, setTweetUrl] = useState<string | null>(null); // State for tweet URL

  // Handle the submission of the tweet URL and set the tweet content based on the URL.
  const handleTweetSubmit = (url: string) => {
    setTweetUrl(url);
    // You can add code to fetch the tweet content here based on the URL.
    // For now, it just uses a placeholder.
    setTweetContent(
      "Apple has announced you can now add California driver's licenses and state IDs to Apple Wallet!"
    );
  };
  ...
  const features = [
    {
      ...
      skeleton: (
        <TwitterSubmission onSubmit={handleTweetSubmit} /> // Pass the handler function
      ),
    ...
    <div className="relative py-10 lg:py-20 max-w-7xl mx-auto font-sans">
      ...
      {tweetUrl && (
        <div className="mt-8 p-4 bg-gray-100 rounded-lg shadow-md">
          <TweetDemo tweetUrl={tweetUrl} /> {/* Pass tweetUrl to TweetDemo */}
        </div>
      )}
    </div>
  );
