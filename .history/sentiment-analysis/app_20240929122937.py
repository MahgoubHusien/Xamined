...
# API Endpoint for Streaming ChatGPT Interaction
@app.route('/chatgpt-stream', methods=['POST'])
def chatgpt_stream():
    data = request.json
    if 'prompt' not in data:
        return jsonify({'error': 'No prompt provided'}), 400
    ...
    return Response(stream_chatgpt(prompt), content_type='text/plain')
